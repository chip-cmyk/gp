...
public class ArContentController extends BaseController
{
     ...
     * 查询AR内容列表
     */
    @PreAuthorize("@ss.hasPermi('ar:arcontent:list')")
    @GetMapping("/list")
     ...
     * 导出AR内容列表
     */
    @PreAuthorize("@ss.hasPermi('ar:arcontent:export')")
     ...
     * 获取AR内容详细信息
     */
    @PreAuthorize("@ss.hasPermi('ar:arcontent:query')")
    @GetMapping(value = "/{contentId}")
    public AjaxResult getInfo(@PathVariable("contentId") Long contentId)
    {
        return success(arContentService.selectArContentByContentId(contentId));
    }

    /**
     * 新增AR内容
     */
    @PreAuthorize("@ss.hasPermi('ar:arcontent:add')")
     ...
     * 修改AR内容
     */
    @PreAuthorize("@ss.hasPermi('ar:arcontent:edit')")
     ...
     * 删除AR内容
     */
    @PreAuthorize("@ss.hasPermi('ar:arcontent:remove')")
    @Log(title = "AR内容", businessType = BusinessType.DELETE)
	@DeleteMapping("/{contentIds}")
    public AjaxResult remove(@PathVariable Long[] contentIds)
    {
        return toAjax(arContentService.deleteArContentByContentIds(contentIds));
    }
}
